...
func choose_discard_index(hand: Array, visible_knowledge) -> Dictionary:
	if hand.is_empty():
		return {"error": ERR_INVALID_PARAMETER}
	var best_index := 0
	var best_score := INF
	for index in hand.size():
		var score := _keep_score(index, hand, visible_knowledge)
...
func _keep_score(index: int, hand: Array, visible_knowledge) -> float:
	...
	for other_index in hand.size():
		...
		if tile.identity.equals(other.identity):
			...
			if distance == 1:
				...
				score += 1.0
	score -= visible_knowledge.seen_identity_count(tile.identity_key()) * 0.25
	return score
