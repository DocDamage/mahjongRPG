...
func _test_finale_checkpoint_standoff_endings_and_credits(failures: Array[String]) -> void:
	...
	if session.finale.record_championship_result(true, session.public_life, session.day) != OK or not session.public_life.has_completed(&"final_championship") or session.brands.match_wins.get(&"texas_king", 0) != 1:
	...
	if restored.restore(round_trip) != OK or not restored.finale.credits_seen or StringName(restored.finale.ending_id) != &"open_hall":
		failures.append("P15 finale checkpoints, ending result, and credits state must round-trip")
...
func _test_postgame_continuity_collections_and_migrations(failures: Array[String]) -> void:
	...
	var round_trip: Dictionary = session.snapshot()
	var restored = GameSessionScript.new()
	if restored.restore(round_trip) != OK or not restored.postgame.is_active() or restored.postgame.ending_provenance != &"open_hall" or not restored.angler.records.has(&"tarpon"):
		failures.append("P16 canonical postgame and collections must round-trip through a long-running save")
	...
	if migrated.restore(pre_p15) != OK or migrated.finale.phase != &"unstarted" or migrated.postgame.is_active():
		failures.append("pre-P15 saves must migrate to safe empty finale and postgame records")
	...
	if migrated_p16.restore(pre_p16) != OK or migrated_p16.finale.credits_seen or migrated_p16.postgame.is_active():
		failures.append("pre-P16 saves must preserve finale state while gaining an inactive postgame record")
...
func _finish_finale(session, response: StringName, failures: Array[String]) -> void:
	if session.finale.begin_championship(session.story, session.public_life) != OK or session.finale.record_championship_result(true, session.public_life, session.day) != OK or session.finale.resolve_standoff(response, session.story) != OK or session.finale.acknowledge_credits() != OK:
	...
	for arc_id_value in session.community.definitions:
		...
		for _step in 3:
			if session.community.advance(arc_id, session.community.expected_action(arc_id), session.relationships, session.helpers).has("error"):
				...
				return null
	for property_id in [&"saints_landing_depot", &"ironhook_customs_warehouse", &"red_testament_expedition", &"bridlewood_water_right", &"gulls_rest_mooring", &"saints_market_charter"]:
		session.properties.outcomes[property_id] = &"quest"
