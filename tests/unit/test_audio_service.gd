...
func run() -> Array[String]:
	...
	if service.load_runtime_catalog() != OK:
		failures.append("runtime audio catalog should load")
	for weather_id in [&"clear", &"rain", &"cloudy", &"thunderstorm", &"dust_wind", &"supernatural_fog"]:
		if not ResourceLoader.exists(service.ambience_path(weather_id)):
			failures.append("P10 weather ambience should resolve %s" % weather_id)
	for event_id in [&"footstep_grass", &"footstep_gravel", &"footstep_wood", &"mahjong_tile", &"fishing_bite", &"fishing_tension"]:
